...
type Todo = { id: string; text: string; completed: boolean; created: string; updated: string; categoryId: string; priority: Priority; effort: number; color: string; scheduledStart: string | null; durationMinutes: number; status: string; content: string; projectId?: string | null; milestoneId?: string | null };
...
type Milestone = { id: string; projectId: string; name: string; targetDate: string | null; completed: boolean; created: string; updated: string };
...
type Workspace = { folders: Folder[]; notes: Note[]; todos: Todo[]; todoCategories: TodoCategory[]; projects: Project[]; milestones: Milestone[]; journalEntries: JournalEntry[]; pins: Pin[]; selectedFolderId: string; selectedNoteId: string; sortMode: SortMode; plannerView: PlannerView };
...
function normalizeWorkspace(input: Partial<Workspace> | null | undefined): Workspace {
  ...
  const todos = (Array.isArray(input?.todos) ? input.todos : []).map((todo) => {
    const projectId = todo.projectId && projectIds.has(todo.projectId) ? todo.projectId : null;
    const milestone = todo.milestoneId ? milestoneById.get(todo.milestoneId) : undefined;
    return { ...todo, categoryId: todo.categoryId || "inbox", priority: (todo.priority || "medium") as Priority, effort: todo.effort || 2, color: todo.color || "", scheduledStart: todo.scheduledStart || null, durationMinutes: todo.durationMinutes || 30, status: todo.status || "todo", content: todo.content || "", projectId, milestoneId: milestone?.projectId === projectId ? milestone.id : null };
  });
  ...
  return { folders, notes, todos, todoCategories: categories, projects, milestones, journalEntries, pins, selectedFolderId: input?.selectedFolderId || "inbox", selectedNoteId: input?.selectedNoteId || "", sortMode, plannerView };
}
...
let projectTab: "overview" | "milestones" = "overview";
...
const projectMilestones = (projectId: string) => state.milestones.filter((milestone) => milestone.projectId === projectId);
const milestoneTasks = (milestoneId: string) => state.todos.filter((todo) => todo.milestoneId === milestoneId);
...
function renderMilestoneCard(milestone: Milestone) {
...
function renderProjectMilestones(project: Project) {
  ...
  const unassigned = projectTasks(project.id).filter((todo) => !todo.milestoneId);
  return `<section class="project-related project-milestones"><header><div><span class="eyebrow">Project plan</span><h2>Milestones</h2></div><button class="secondary-button" id="add-project-milestone"><i class="ph ph-plus"></i>Add milestone</button></header><div class="milestone-list">${milestones.map(renderMilestoneCard).join("") || '<div class="project-detail-empty milestone-empty"><i class="ph ph-flag"></i><strong>No milestones yet</strong><span>Add the first outcome this project is working toward.</span></div>'}</div><section class="unassigned-tasks"><header><div><h3>No milestone</h3><p>Tasks that still belong to the project but are not attached to a milestone.</p></div><button class="secondary-button" id="add-unassigned-project-task"><i class="ph ph-plus"></i>Add task</button></header><div class="project-task-list">${unassigned.map(renderProjectTask).join("") || '<div class="milestone-task-empty">No unassigned tasks.</div>'}</div></section></section>`;
}
function renderProjectDetail(project: Project) {
  const meta = projectStatusMeta[project.status]; const milestones = projectMilestones(project.id); const openMilestones = milestones.filter((milestone) => !milestone.completed);
  const milestoneProgress = milestones.length ? Math.round((milestones.length - openMilestones.length) / milestones.length * 100) : 0;
  const milestonesContent = renderProjectMilestones(project);
  const overview = `<div class="project-overview-grid"><section class="project-description-card"><label for="project-description">Description</label><textarea id="project-description" placeholder="Add a description…">${escapeHtml(project.description)}</textarea></section><aside class="project-progress-card"><span class="eyebrow">Progress</span><strong>${milestoneProgress}%</strong><div><span style="width:${milestoneProgress}%"></span></div><p>${openMilestones.length} open · ${milestones.length - openMilestones.length} completed</p></aside></div>${milestonesContent}`;
  return `<main class="project-detail-view" aria-label="${attr(project.name)} project"><header class="project-detail-topbar"><button id="back-to-projects"><i class="ph ph-arrow-left"></i>Projects</button><span><i class="ph ph-cube"></i>${escapeHtml(project.name)}</span><button class="icon-button" id="project-more" aria-label="Project actions"><i class="ph ph-dots-three"></i></button></header><nav class="project-detail-tabs" aria-label="Project sections"><button class="${projectTab === "overview" ? "is-active" : ""}" data-project-tab="overview">Overview</button><button class="${projectTab === "milestones" ? "is-active" : ""}" data-project-tab="milestones">Milestones <span>${milestones.length}</span></button></nav><div class="project-detail-scroll"><article class="project-hero"><span class="project-hero-mark ${meta.tone}"><i class="ph ph-cube"></i></span><div class="project-name-editor" id="project-name" contenteditable="true" role="textbox" aria-multiline="false" aria-label="Project name">${linkedPlainText(project.name)}</div><p>${escapeHtml(project.summary || "Add a short summary so everyone knows what success looks like.")}</p><div class="project-properties"><label><span>Status</span><i class="ph ${meta.icon}"></i><select id="project-status">${projectStatuses.map((status) => `<option value="${status}" ${status === project.status ? "selected" : ""}>${projectStatusMeta[status].label}</option>`).join("")}</select></label><label><span>Target date</span><i class="ph ph-calendar-blank"></i><input id="project-target-date" type="date" value="${attr(project.targetDate || "")}"></label><span class="project-task-stat"><i class="ph ph-flag"></i>${openMilestones.length} open milestones</span></div></article>${projectTab === "overview" ? overview : milestonesContent}</div></main>`;
}
...
async function createMilestone(project: Project) {
...
function bindTaskDetailEvents() {
  ...
  document.querySelectorAll<HTMLElement>("[data-task-prop]").forEach((el) => el.addEventListener("change", () => {
    ...
    if (prop === "effort" || prop === "durationMinutes") (todo as any)[prop] = Number(value);
      ...
      todo.projectId = value || null;
      if (!state.milestones.some((milestone) => milestone.id === todo.milestoneId && milestone.projectId === todo.projectId)) todo.milestoneId = null;
      todo.updated = now(); void saveState(false); renderApp(); return;
    }
    else if (prop === "milestoneId") {
      const milestone = state.milestones.find((item) => item.id === value);
      todo.milestoneId = milestone?.id || null;
      if (milestone) todo.projectId = milestone.projectId;
    }
